...
// actual quote asset (BI2XUSD for both spot and futures) — then records each
...
// market — spot and futures alike — quotes and collateralizes in BI2XUSD, the
// platform's internal stable currency; futures collateral used to be real
// USDC, now converts to/settles in BI2XUSD like everything else on the
...
func (s *Service) Create(ctx context.Context, base string, market models.Market, symbol string, cfg map[string]string) (*models.MarketMaker, error) {
	...
	// mixed-case tickers ("CrudeOIL-BI2XUSD", "AAPL.us-BI2XUSD").
	//
	// An OPTIONS row in symbol_configs is keyed by the UNDERLYING spot pair
	// (e.g. "BTC-BI2XUSD"), not any single option contract — each strike/
...
// asset symbol for this desk (e.g. "BTC" / "BI2XUSD").
//
